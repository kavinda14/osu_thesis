...
class Simulator:
    ...
    def run(self, neural_model, curr_robot_positions, device, obs_occupied_oracle=set(), train=False, generate_data=False):
        self.iterations += 1        
        ...
        if self.planner == 'mcts':
            ...
            solution, solution_locs, root, list_of_all_nodes, winner_node, winner_loc = mcts.mcts(budget, max_iterations, exploration_exploitation_parameter, self.robot, self.sensor_model, self.map, self.rollout_type, self.reward_type, neural_model, device=device)
