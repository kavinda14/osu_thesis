...
def generate_valid_neighbors(current_state, state_sequence, robot):
    ...
    for action in actions:
        valid, new_loc = robot.check_valid_move_mcts(action, current_loc)
...
def mcts(budget, max_iterations, exploration_exploitation_parameter, robot, input_map):
    ...
    for iter in range(max_iterations):
        ...
        rollout_sequence = rollout(subsequence=current.sequence, budget=budget)
        # rollout_reward = reward(action_sequence=rollout_sequence)
        rollout_reward = reward(rollout_sequence)
